...
class ImageFileDirectory_v2(MutableMapping):
    ...
    def __init__(self, ifh=b"II\052\0\0\0\0\0", prefix=None, group=None):
        ...
        if ifh[:4] not in PREFIXES:
            raise SyntaxError("not a TIFF file (header %s not valid)" % (repr(ifh)))
    ...
    def _setitem(self, tag, value, legacy_api):
        ...
        if not is_ifd and (
            ...
            try:
                ...
                warnings.warn(
                    "Metadata Warning, tag %s had too many entries: %d, expected 1" % (tag, len(values))
                )
                dest[tag] = values[0]
    ...
    def load(self, fp):
        ...
        try:
            for i in range(self._unpack("H", self._ensure_read(fp, 2))[0]):
                ...
                msg = "tag: %s (%s) - type: %s (%s)" % (tagname,tag,typename,typ)

                try:
                    unit_size, handler = self._load_dispatch[typ]
                except KeyError:
                    logger.debug(msg + " - unsupported type %s" % (typ))
                    continue  # ignore unsupported type
                size = count * unit_size
                if size > 4:
                    here = fp.tell()
                    (offset,) = self._unpack("L", data)
                    msg += " Tag Location: %s - Data Location: %s" % (here, offset)
                    fp.seek(offset)
                ...
                if len(data) != size:
                    warnings.warn(
                        ...
                        "Expecting to read %s bytes but only got %d. Skipping tag %s" % (size, len(data), tag)
                    )
                    logger.debug(msg)
                    continue
    ...
    def tobytes(self, offset=0):
        ...
        for tag, value in sorted(self._tags_v2.items()):
            ...
            msg = "save: %s (%s) - type: %s (%s)" % (tagname, tag, typname, typ)
            msg += " - value: " + (
        ...
        for tag, typ, count, value, data in entries:
            logger.debug("%s %s %d %s %s" % (tag, typ, count, repr(value), repr(data)))
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        logger.debug("- __first: %d" % (self.__first))
    ...
    def _seek(self, frame):
        ...
        while len(self._frame_pos) <= frame:
            ...
            logger.debug(
                "Seeking to frame %s, on frame %s, " % (frame, self.__frame)
                + "__next %d, location: %d" % (self.__next, self.fp.tell())
            )
    ...
    def _load_libtiff(self):
        ...
        try:
            ...
            raise OSError(e, "Couldn't set the image")
    ...
    def _setup(self):
        ...
        logger.debug("- YCbCr subsampling: %s" % (self.tag.get(530)))
        ...
        try:
            ...
            raise SyntaxError(e, "unknown pixel mode")
...
def _save(im, fp, filename):
    ...
    try:
        ...
        raise OSError(e, "cannot write mode %s as TIFF" % (im.mode))
    ...
    if libtiff:
        ...
        while True:
            ...
            if s:
                break
        if s < 0:
            raise OSError("encoder error %s when writing image file" % (s))
...
class AppendingTiffWriter:
    ...
    def rewriteLastShortToLong(self, value):
        ...
        if bytesWritten is not None and bytesWritten != 4:
            raise RuntimeError("wrote only %s bytes but wanted 4" % (bytesWritten))

    def rewriteLastShort(self, value):
        self.f.seek(-2, os.SEEK_CUR)
        bytesWritten = self.f.write(struct.pack(self.shortFmt, value))
        if bytesWritten is not None and bytesWritten != 2:
            raise RuntimeError("wrote only %s bytes but wanted 2" % (bytesWritten))
